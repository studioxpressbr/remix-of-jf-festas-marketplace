...
// Price per credit in cents (R$ 2.00)
...
serve(async (req) => {
  ...
  try {
    ...
    logStep("Processing credit purchase", { quantity });
      ...
      line_items: [
        {
          price_data: {
            currency: 'brl',
            product_data: {
              name: 'Créditos para Leads',
              description: `${quantity} crédito${quantity > 1 ? 's' : ''} para liberar contatos de clientes`,
            },
